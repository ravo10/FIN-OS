    ...
    if Player and Player:IsValid() then
        ...
        if PHYSICSPROPERTIESSTABLE and PHYSICSPROPERTIESSTABLE["FinBeingTracked"] and PHYSICSPROPERTIESSTABLE["FinBeingTracked"]:IsValid() then
            ...
            draw.RoundedBox( 8,
                ...
                width,
                77,
                Color( 255, 255, 0, 160 ) -- GreenYellow
            )
        ...
        if ENT and ENT:IsValid() and Player and Player:IsValid() and Player:GetActiveWeapon():IsValid() and Player:GetActiveWeapon():GetClass() == "fin_os" then
                ...
                for k, _ in pairs( FinAreaPointsTable ) do
                    ...
                    if k >= 3 then
                        ...
                        render.DrawQuad(
                            ...
                            ENT:LocalToWorld( FinAreaPointsTable[ 1 ] ),
                            ENT:LocalToWorld( FinAreaPointsTable[ k - 1 ] ),
                            ENT:LocalToWorld( FinAreaPointsTable[ k ] ),
                            ENT:LocalToWorld( FinAreaPointsTable[ 1 ] ),
                            Color( 255, 255, 255 )
                        )
            ...
            for k, v in pairs( FinAreaPointsTable ) do
                ...
                if point1 and point2 then
                    ...
                    point2 = ENT:LocalToWorld( FinAreaPointsTable[ k + 1 ] )

                    render.DrawLine( point1, point2, Color( 255, 121, 0 ), true )

                    if ( k + 1 ) == #FinAreaPointsTable then

                        render.DrawLine( point2, ENT:LocalToWorld( FinAreaPointsTable[ 1 ] ), Color( 255, 121, 0 ), true )
